...
@Component({
  selector: 'app-profil-turiste',
  standalone: true,
  imports: [RouterOutlet,FormsModule,RouterModule],
  templateUrl: './profil-turiste.component.html',
  styleUrl: './profil-turiste.component.css'
})
export class ProfilTuristeComponent implements OnInit {
  ...
  promeni(){
    ...
    (this.user.brojKredinteKartice == this.userNov.brojKredinteKartice)){
      this.userSerivce.promeni(this.userNov).subscribe(data=>{
        this.slikaUrl = "/uploads/" + this.userNov.slika
        this.ngOnInit()
      })
    } else {
